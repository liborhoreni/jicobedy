...
const MENU_SCHEMA = {
  ...
  required: ['isMenu', 'soups', 'meals', 'weekly'],
  additionalProperties: false,
};
...
- Pokud příspěvek NENÍ jídelní menu nebo den "${dayName}" v něm není, vrať isMenu=false a prázdná pole.` });
  ...
  try {
    ...
    return { soups: norm(json.soups), meals: norm(json.meals), weekly: norm(json.weekly) };
  } catch {
    return null;
  }
}
...
export async function fetchQwertyMenu(dateOverride) {
  ...
  for (const { p, imgUrl } of withPhoto) {
    ...
    if (menu && (menu.soups.length || menu.meals.length || menu.weekly.length)) {
      return { menu, scrapedAt: new Date().toISOString() };
    }
  }
  return null;
}
