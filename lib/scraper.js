...
function parseMenickaCz(html, restaurant) {
  ...
  const datumDivs = $('div.obsah div.datum');
  datumDivs.each((i, datumEl) => {
    if (todayMenu) return;
    const datumText = $(datumEl).text();
    if (!datumText.includes(dateStr)) return;

    const menu = { soups: [], meals: [] };

    let el = $(datumEl).next();
    while (el.length && !el.hasClass('datum')) {
      const items = el.find('li.polevka, li.jidlo');
      const directItems = el.is('li.polevka') || el.is('li.jidlo') ? el : $();

      items.add(directItems).each((j, li) => {
        const $li = $(li).clone();
        const price = $li.find('.cena').text().trim();
        $li.find('.cena').remove();
        $li.find('.foto').remove();
        const name = $li.text().trim().replace(/\s+/g, ' ');
