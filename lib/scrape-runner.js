...
- "allergens" = čísla alergenů u dané položky jako čárkou oddělený seznam, např. "1, 3, 7" (jen čísla, bez písmene "A" a bez slova "alergeny"). Pokud u položky alergeny nejsou uvedené, vrať prázdný string ""
- Pokud den "${dayName}" v menu není, vrať null pro soup i meal` },
  ...
  try {
    ...
    const allergens = (a) => (a || '').trim();

    if (json.soup && json.soup.name) {
      menu.soups.push({ name: json.soup.name, price: cleanPrice(json.soup.price), allergens: allergens(json.soup.allergens) });
    }
    if (json.meal && json.meal.name) {
      menu.meals.push({ name: json.meal.name, price: cleanPrice(json.meal.price), allergens: allergens(json.meal.allergens) });
    }
    if (Array.isArray(json.weekly)) {
      for (const item of json.weekly) {
        if (item.name) menu.weekly.push({ name: item.name, price: cleanPrice(item.price), allergens: allergens(item.allergens) });
      }
    }
